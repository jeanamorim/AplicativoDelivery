  ...
  return (
    <>
      <Input
        ...
        ref={addressesRef}
        isFocused={() => addressesRef.current.isFocused()}
        onChangeText={text => onChangeText('addresses', text)}
        onBlur={() => onBlur('addresses')}
        value={form.addresses}
        returnKeyType="next"
        onSubmitEditing={() => postalCodeRef.current.focus()}
        error={
          fieldErrors.addresses && touched.addresses && fieldErrors.addresses
        }
      />
      <Input
        editable={!lockForm}
